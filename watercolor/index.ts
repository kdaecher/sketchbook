...
interface ShaderLocations {
  ...
  wetness: WebGLUniformLocation | null;
  bleeding: WebGLUniformLocation | null;
  brushPos: WebGLUniformLocation | null;
  brushSize: WebGLUniformLocation | null;
  time: WebGLUniformLocation | null;
  isDrawing: WebGLUniformLocation | null;
}

type ToolType = 'brush' | 'sponge';

class WatercolorSketchbook {
  private canvas: HTMLCanvasElement;
  private gl: WebGLRenderingContext;
  private currentColor: [number, number, number, number];
  private brushSettings: BrushSettings;
  private isDrawing: boolean;
  private lastPos: Point | null;
  private brushPos: Point | null;
  ...
  private canvasTexture: WebGLTexture | null;
  private framebuffer: WebGLFramebuffer | null;
  private startTime: number;

  constructor() {
      this.canvas = document.getElementById('watercolorCanvas') as HTMLCanvasElement;
      ...
      this.program = null;
      this.locations = {} as ShaderLocations;
  ...
  private setupUI(): void {
      ...
      colors.forEach((color, index) => {
          ...
          if (index === 1) swatch.classList.add('active'); // Blue default
      ...
      const wetnessSlider = document.getElementById('wetness') as HTMLInputElement;
      const wetnessValue = document.getElementById('wetnessValue') as HTMLElement;
      ...
      const bleedingSlider = document.getElementById('bleeding') as HTMLInputElement;
      const bleedingValue = document.getElementById('bleedingValue') as HTMLElement;
      ...
      // Tool buttons
      const brushTool = document.getElementById('brushTool') as HTMLButtonElement;
      const spongeTool = document.getElementById('spongeTool') as HTMLButtonElement;
  ...
  private render(): void {
      ...
      if (!this.program || !this.canvasTexture) return;
      ...
      if (this.brushPos) {
          gl.uniform2f(this.locations.brushPos, 
              this.brushPos.x, this.canvas.height - this.brushPos.y);
      }
