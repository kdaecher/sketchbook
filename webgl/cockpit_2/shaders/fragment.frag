...
void main(void){
    ...
    vec3 color = vec3((sin(row+1.0)), 0.0, sin(col+1.0));
    
    vec3 smallCircles = circleOutline(st, vec2(0.5), 0.5, color, 0.03) * color;

    st = rotate2D(st, u_time * 8.0 * PI);

    float drawCross = cross(st, 0.6);

    vec3 comp = smallCircles + vec3(drawCross);
